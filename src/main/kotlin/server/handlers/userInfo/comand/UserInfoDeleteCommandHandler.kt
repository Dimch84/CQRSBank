...
@Component
class UserInfoDeleteCommandHandler @Autowired constructor(observerUserInfo: ObserverUserInfo,
    ...
    override fun handle(simpleCommand: SimpleCommand): String {
        val event = (simpleCommand.store.command as UserInfoDeleteCommand).event
        userInfoEvents(event.id, throw_else=true).run {
